<x-app-layout>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    ...
                    <form action="{{ route('list_program.update', $program->id_programs) }}" method="POST"
                        ...
                        <div class="mb-6">
                            ...
                            <textarea id="" rows="4" name="front_description"
                                ...
                                placeholder="Masukan Deskripsi Depan">{{ $program->front_description }}</textarea>
                        </div>
                        <div class="mb-6">
                            ...
                            <textarea id="message" rows="4" name="main_description"
                                class="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500 "
                                placeholder="Masukan Deskripsi Utama">{{ $program->main_description }}</textarea>
                        </div>
                            ...
                            <table id="table-variation"
                                ...
                                <tbody>
                                    @foreach ($program->variations as $variation)
                                        <tr>
                                            <td>
                                                <input type="hidden" name="id_variation[]"
                                                    value="{{ $variation->id_variation }}">
                                                ...
                                                <select id="id_level" name="id_level[]"
                                                    ...
                                                    @foreach ($levels as $level)
                                                        <option value="{{ $level->id_levels }}"
                                                            {{ $variation->id_level == $level->id_levels ? 'selected' : '' }}>
                                                            {{ $level->level_name }}
                                                        </option>
                                                    @endforeach
                                                </select>
                                                ...
                                                <select id="id_room" name="id_room[]"
                                                    ...
                                                    @foreach ($rooms as $room)
                                                        <option value="{{ $room->id_room }}"
                                                            {{ $variation->id_room == $room->id_room ? 'selected' : '' }}>
                                                            {{ $room->room_name }}
                                                        </option>
                                                    @endforeach
                                                </select>
                                                ...
                                                <select id="id_time" name="id_time[]"
                                                    ...
                                                    @foreach ($times as $time)
                                                        <option value="{{ $time->id_time }}"
                                                            {{ $variation->id_time == $time->id_time ? 'selected' : '' }}>
                                                            @if ($time->day == 'day1')
                                                                Senin - Kamis
                        ...
                        <div class="mb-6">
                            ...
                            <textarea id="message" rows="4" name="senior_description"
                                ...
                                placeholder="Kosongkan bila tidak ada">{{ $program->senior_description }}</textarea>
                        </div>
                        <div class="mb-6">
                            <label for="base-input" class="block mb-2 text-sm font-medium text-dark ">Deskirpsi
                                SMP</label>
                            <textarea id="message" rows="4" name="junior_description"
                                class="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500 description_school"
                                placeholder="Kosongkan bila tidak ada">{{ $program->junior_description }}</textarea>
                        </div>
                        <div class="mb-6">
                            <label for="base-input" class="block mb-2 text-sm font-medium text-dark ">Deskirpsi
                                SD</label>
                            <textarea id="message" rows="4" name="elementary_description"
                                class="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500 description_school"
                                placeholder="Kosongkan bila tidak ada">{{ $program->elementary_description }}</textarea>
                        </div>
    ...
    @push('scripts')
        ...
        <script>
            ...
            function addRow() {
                ...
                cellLevel.innerHTML = `
        <select name="new_variation[${rowCount}][id_level]"
            class="bg-white border border-gray-300 bg-gray-50 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 "
            required>
            <option value="" selected>Pilih Kelas</option>
            @foreach ($levels as $level)
                <option value="{{ $level->id_levels }}">{{ $level->level_name }}</option>
            @endforeach
        </select>
    `;
                cellRoom.innerHTML = `
        <select name="new_variation[${rowCount}][id_room]"
            class="bg-white border border-gray-300 bg-gray-50 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 "
            required>
    ...
    `;
                cellTime.innerHTML = `
        <select name="new_variation[${rowCount}][id_time]"
            class="bg-white border border-gray-300 bg-gray-50 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 "
            required>
    ...
    `;
                cellPrice.innerHTML = `
        <input type="text" name="new_variation[${rowCount}][price]"
            class="bg-white border border-gray-300 bg-gray-50 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 "
            placeholder="Rp 20000" required>
    `;
                cellCapacity.innerHTML = `
        <input type="text" name="new_variation[${rowCount}][capacity]"
            class="bg-white border border-gray-300 bg-gray-50 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 "
            placeholder="20" required>
    `;
    ...
    `;
            ...
            function deleteRowTapproval(button) {
                ...
                rowCount--; // Update rowCount to the correct value
            }
        </script>
    @endpush
</x-app-layout>
